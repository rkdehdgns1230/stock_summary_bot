...
def build_prompt(today: str, persona_instruction: str, us_data: str,
                 commodities_data: str, kospi_data: str, kosdaq_data: str,
                 fng_stage: str, score: int, news_data: str) -> str:
    """Gemini에 전달할 프롬프트 구성"""
    return f"""
[AI 페르소나 지침]
{persona_instruction}
...
Step 3. WTI 원유와 금 가격의 방향성이 글로벌 인플레이션 기대치 및 리스크온/리스크오프 심리에 미치는 영향을 파악하고, 달러 인덱스(DXY) 강도가 신흥국(한국 포함) 외국인 자금 유출입에 미치는 영향을 판단한다.
Step 4. 공포·탐욕 지수가 현재 시장 심리의 과열·공포 구간 여부를 나타내는지 해석하고, 반전 가능성을 고려한다.
Step 5. 뉴스 헤드라인에서 당일 한국 증시에 직접 영향을 줄 핵심 이슈(정책, 실적, 지정학적 리스크 등)를 추출한다.
Step 6. 위 다섯 가지 요소를 종합하여 오늘 코스피 및 코스닥의 상승/하락 가능성과 강도를 결론 짓는다.
Step 7. 특정 데이터가 누락된 경우, 다른 지표들을 근거로 삼아 그 항목을 최대한 추론·보완한다.
...
2. __🌏 글로벌 시장 동향__
   수집된 수치 데이터(지수값, 환율, 금리, 원자재)는 아래처럼 코드 블록으로 묶을 것:
   ```
   나스닥: 18,234.56 (+2.35%)
...
def generate_report(today: str, score: int, fng_stage: str,
                    ...
                    kospi_data: str, kosdaq_data: str, news_data: str) -> str:
   ...
   #  persona_instruction = get_persona_instruction(score)
    ...
    prompt = build_prompt(today, market_expert_guidelines, us_data, commodities_data, kospi_data, kosdaq_data, fng_stage, score, news_data)
