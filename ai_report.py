...
3. __🌏 시장 스냅샷__
   ...
   ```
   테이블 이후: 오늘 국장에 가장 직접 영향을 미치는 수치 1개를 인용 블록 한 줄로.
   >**나스닥 +2.35%** — 국장에 어떤 의미인지 친구한테 설명하듯 한 문장.
   특정 가격 레벨 언급 시 `inline code` 사용.  예) `2,600p` 지지 여부

4. __🔥 오늘의 핵심__
   [뉴스] 오늘 시장에 영향 줄 뉴스 최대 3개를 불렛(-)으로.
   형식: _뉴스 제목 요약_ → **시장 영향 한 줄** 로 구성.
   (예: _연준 위원 매파 발언_ → **금리 인하 기대 꺾여, 성장주 압박 예상**)

   [섹터] 뉴스 불렛 이후, 아래 두 줄은 항상 필수 출력:
   - **수혜 섹터:** [섹터명] — 오늘 상대적으로 유리한 이유 한 줄 (대표 종목 1개 언급 허용)
   - **주의 섹터:** [섹터명] — 오늘 상대적으로 불리한 이유 한 줄

5. __🎯 결론 & 대응__
   첫 줄: 방향 결론을 인용 블록으로 선언.
   >코스피 **상승 확률 약 X%** — 근거: [핵심 요인 1~2개] (수급 흐름 포함)
   "상승 예상" 단독 표현 금지. 반드시 X% 숫자로 확률 명시.
   확률 기준: 65% 이상=높은 확신 / 45~64%=보통 / 44% 이하=하락 우세.

   이어서 관전 포인트 불렛 2개:
   👀 각 포인트에 가격 레벨을 `inline code`로 필수 포함.
   레벨 뒤에 맥락(전고점·전저점·지지선·저항선 대비) 한 구 추가 필수.
   예) `2,550p` — 지난주 저점 대비 이탈 시 하락 가속 가능

   행동 가이드:
   - 📌 포지션: 매수 확대 / 관망 / 비중 축소 중 하나 명시
   - 🛡️ 리스크 관리 — 구체적 레벨은 `inline code`로  예) `2,580p` 이탈 시 손절

   전망을 뒤집을 변수 1~2개:
   - ⚠️ **변수** → 이렇게 되면 이런 시나리오. "이 정도는 알고 가자" 톤.

   마지막 한 줄: ||오늘의 최종 결론 — 클릭해서 확인||

[Markdown 규칙 — 텔레그램 전용]
후처리가 **text** → MarkdownV2 *bold* 로 자동 변환하므로 특수문자 이스케이프 불필요.

서식별 용도 (언제 쓸지가 핵심):
- __텍스트__   섹션 헤더 전용 (밑줄). 섹션 헤더 외에는 절대 사용 금지.
- **텍스트**   핵심 수치·방향·결론을 문장 안에서 강조 (굵게). 남발 금지, 문단당 1~2개.
- _텍스트_     뉴스 제목·부연 설명·보조 맥락에 사용 (기울임).
- `텍스트`     개별 지표값·가격 레벨·퍼센트 수치 표기에 사용 (인라인 코드).
               예) `VIX 22.3`, `2,600p`, `+2.35%`, `4.25%`
- >텍스트      핵심 인사이트·방향 결론·판정 한 줄을 강조할 때 사용 (인용 블록, 좌측 파란 바).
               줄 맨 앞에 > 를 붙인다. 연속된 줄에 붙이면 멀티라인 블록.
- ||텍스트||   탭해야 열리는 스포일러. 섹션 5 최종 결론 한 줄에만 사용.
- ~~텍스트~~   빗나간 예측 요소를 시각적으로 표시할 때만 사용 (취소선).
               예) ~~코스피 상승 예상~~ → 실제 하락
- ```블록```   지표 정렬 테이블 전용. 서술 텍스트에는 사용 금지.
- 절대 금지    *텍스트* (별표 1개), ### 헤더
