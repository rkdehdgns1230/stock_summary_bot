...
3. __🌏 글로벌 시장 동향__
   ...
   ```
   테이블 이후: 가장 눈에 띄는 수치 1개와 국장 영향을 인용 블록 한 줄로.
   >**나스닥 +2.35%** — 국장에 어떤 의미인지 친구한테 설명하듯 한 문장.
   특정 가격 레벨 언급 시 `inline code` 사용.  예) `2,600p` 지지 여부

4. __📰 핵심 뉴스 & 이슈__
   오늘 시장에 영향 줄 뉴스 최대 3개를 불렛(-)으로.
   형식: _뉴스 제목 요약_ → **시장 영향 한 줄** 로 구성.
   (예: _연준 위원 매파 발언_ → **금리 인하 기대 꺾여, 성장주 압박 예상**)

5. __🔮 오늘의 증시 전망__
   첫 줄: 방향 결론을 인용 블록으로 선언.
   >코스피 **상승/하락/혼조** 예상 — 핵심 근거 한 줄 (수급 흐름 포함)
   이어서 보조 설명 1~2문장 (결론 뒷받침, 섹터 언급 포함).
   - **오늘 국장 관전 포인트:**
     👀 불렛 2개만, 주목할 가격 레벨은 `inline code`로  예) `2,630p` 돌파 여부
