...
def build_prompt(today: str, persona_instruction: str, us_data: str,
                 kospi_data: str, fng_stage: str, score: int, news_data: str) -> str:
    """Gemini에 전달할 프롬프트 구성"""
    return f"""
[페르소나]
{persona_instruction}
...
def generate_report(today: str, score: int, fng_stage: str,
                    us_data: str, kospi_data: str, news_data: str) -> str:
    """Gemini API를 호출하여 모닝 브리핑 리포트 텍스트 반환"""
    persona_instruction = get_persona_instruction(score)
    prompt = build_prompt(today, persona_instruction, us_data, kospi_data, fng_stage, score, news_data)
