...
def sanitize_for_telegram_mdv2(text: str) -> str:
    """
    AI 생성 텍스트의 MarkdownV2 특수문자를 이스케이프 처리.
    *bold*, _italic_, `code` 포맷팅 스팬은 보존하고,
    일반 텍스트 구간의 특수문자만 이스케이프한다.
    """
    ...
    for m in FMT_RE.finditer(text):
        ...
        result.append(ESCAPE_RE.sub(r'\\\1', plain))
        result.append(m.group())
        last = m.end()
    result.append(ESCAPE_RE.sub(r'\\\1', text[last:]))
    return ''.join(result)
...
- 일반 텍스트 구간의 특수문자 (예: + - . ( ) ~ ! = # > | { } [ ]) 는 백슬래시로 이스케이프할 것 (예: \+2\.35\%, \(나스닥\))
- 열리는 기호와 닫히는 기호는 반드시 짝을 맞출 것 (미짝 기호가 있으면 파싱 오류 발생)
...
"""
    ...
    payload_photo = {'chat_id': CHAT_ID, 'caption': f'📊 시장 심리: 공포 & 탐욕 지수 (8AM)', 'parse_mode': 'MarkdownV2'}
